...
export interface VLCPlayerProps {
  ...
  onPictureInPictureChange?: (pipData: { isPictureInPicture: boolean }) => void;
  ...
  // Callbacks pour les méthodes d'information
  /** Appelé en réponse à getCurrentTime() */
  onGetCurrentTime?: (data: { currentTime: number }) => void;
  /** Appelé en réponse à getDuration() */
  onGetDuration?: (data: { duration: number }) => void;
  /** Appelé en réponse à getState() */
  onGetState?: (data: { state: PlayerState }) => void;
  /** Appelé en réponse à getVolume() */
  onGetVolume?: (data: { volume: number }) => void;
  /** Appelé en réponse à isMuted() */
  onIsMuted?: (data: { muted: boolean }) => void;
  /** Appelé en réponse à getRate() */
  onGetRate?: (data: { rate: number }) => void;
}
